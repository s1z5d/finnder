...
class Zomato:
    ...
    def parse(self,toFile,endpoint,parameters=None):
        ...
        self._execute(endpoint.lower(),all_parameters, toFile)

    def _execute(self,endpoint,parameter, toFile):
        ...
        if self.response_content_type == "application/json":
            ...
            try:
                ...
                dir_path = os.path.join("components")
                with open (os.path.join(dir_path, toFile), 'w') as fp:
                    json.dump(json_data["restaurants"],fp)
